...
//     ImGui::Inpu("Control Pointer Selection Mode", ImGuiDataType_S8, &SelectionModeString[cp_selection_mode], 0.1, 0, 0, "%.4f");
//     ImGui::PopItemWidth();
...
bool Window::onMouseMoved()
{
    if (mousePressed && moveMode) {
        ...
        projectOnMoveDirection(projPoint);
        projPoint.normalize();
        scene.mesh.moveControlPoints(projPoint);
        scene.meshArap.guiToAlgorithm(scene.mesh.C);
        scene.meshArap.ARAP(1);
